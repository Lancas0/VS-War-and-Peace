...
public class ShellTriggerHandler {
    ...
    public boolean tryTrigger(Dest<Double> totalEnergyDest, Dest<ProjectileShipWrapper> projectileDest) {
        ...
        ProjectileShipWrapper projectile = makeProjectileShip(propellantEndBpDest.get());
        if (projectile == null) {
    ...
    private ProjectileShipWrapper makeProjectileShip(BlockPos propellantEndBp) {
        //ShipBuilder shipBuilder = ShipPool.getOrCreatePool(level).getOrCreateShipBuilder(ShipBuilder::resetBlocks);/
        //don't use pool, todo need to really clean the ship.
        ...
        return ProjectileShipWrapper.ofIfValid(shipBuilder.get(), primerDir);
    }
    /*
