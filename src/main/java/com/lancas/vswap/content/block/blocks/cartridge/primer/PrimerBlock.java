...
public class PrimerBlock extends BlockPlus implements IPrimer/*, IBE<PrimerBE>*/  {
    public static BlockState faceTo(Direction dir) {
        return WapBlocks.Cartridge.Primer.PRIMER.getDefaultState().setValue(DirectionAdder.FACING, dir);
    }
    ...
    public @Nullable BlockClusterData fire(ServerLevel level, List<LoadedMunitionData> loaded, Dest<Double> speDest, List<ItemStack> munitionRemains) {
        ...
        //BlockClusterData fireShipBlockData = new BlockClusterData();
        AtomicBoolean isPropellantPart = new AtomicBoolean(true);
        Vector3i curPos = new Vector3i(IBreech.LOADED_MUNITION_ORIGIN);

        HashMap<UUID, BlockClusterData> eachMunitionRemain = new HashMap<>();
        IPrimer.foreachMunition(level, loaded, PrimerBlock.class, (ship, locPos, state) -> {
            BlockClusterData remainBlockData = eachMunitionRemain.computeIfAbsent(ship.getUuid(), k -> new BlockClusterData());
            //var curShipBlockWriter = ship.getBlockCluster().getDataWriter();
            Block curBlock = state.getBlock();

            if (isPropellantPart.get()) {
                if (curBlock == this) {
                    remainBlockData.setBlock(locPos, this.getTriggeredState(state), false);
                    //curShipBlockWriter.setBlock(locPos, this.getTriggeredState(state), false);
                    return;
                }

                if (curBlock instanceof IPropellant propellant) {
                    speDest.update(x -> x + propellant.getSPE(state));
                    remainBlockData.setBlock(locPos, propellant.getEmptyState(state), false);
                    //curShipBlockWriter.setBlock(locPos, propellant.getEmptyState(state), false);
                    return;
                }

                isPropellantPart.set(false);
            }
        ...
        eachMunitionRemain.values().stream()
            .filter(x -> x.getBlockCnt() > 0)
            .forEach(bc ->
                munitionRemains.add(Docker.stackOfSaBlockData(level, bc))
            );

        return blockData.getBlockCnt() > 0 ? blockData : null;
    }
