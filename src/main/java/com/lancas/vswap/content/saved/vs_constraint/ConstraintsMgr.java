...
public class ConstraintsMgr extends SavedData {
    ...
    public static Integer addConstraint(ServerLevel level, String key, VSConstraint constraint) {
        ...
        EzDebug.log("new constraint id:" + addID);
        //fail to create the constraint
        if (addID == null) {
            EzDebug.warn("fail to add constraint");
            return null;
        }
    ...
    public static boolean removeAnyWithKey(String key) {
        AtomicBoolean anyRemoved = new AtomicBoolean(false);
        allMgr.values().forEach(m -> {
            if (m.removeInLevelOrAddingConstraintImpl(key)) {
                EzDebug.log("remove constraint " + key);
                anyRemoved.set(true);
            }
